...
public class KGraphRepository implements IKGraphRepository {
    ...
    public GraphPageRecord<HashMap<String, Object>> getPageDomain(GraphQuery queryItem) {
        ...
        try {
            String totalCountquery = "MATCH (n) RETURN count( labels(n)) as count";
            int totalCount = 0;
            ...
            if (totalCount > 0) {
                ...
                String domainSql = String.format(
                        "START n=node(*)  RETURN  labels(n) as domain,count(n) as nodecount order by nodecount desc SKIP %s LIMIT %s",
     ...
     */
    @Override
    public HashMap<String, Object> getdomaingraph1(GraphQuery query) {
        ...
        try {
            ...
            if (!StringUtil.isBlank(domain)) {
                ...
                if (!StringUtil.isBlank(query.getNodename()) || !StringUtil.isBlank(cqr)) {
                    ...
                    String nodeSql = "";
                    //MATCH n=(a:法医临床学)-->() where a.name = '%s' RETURN n limit %s
                    if (StringUtil.isBlank(cqWhere)){
                         nodeSql = String.format("MATCH n=(a:法医临床学)-->() where a.name = '%s' RETURN n limit %s", domain,
                                query.getPageSize());
//                    String nodeSql = String.format("MATCH (n:`%s`) <-[r]->(m) %s return * limit %s", domain, cqWhere,
//                            query.getPageSize());
//                    String nodeSql = String.format("MATCH n=(a:'%s')-->() where a.name = '%s' RETURN n limit %s", domain, cqWhere,
////                            query.getPageSize());
                    }else {
                         nodeSql = String.format("MATCH (n:%s) %s RETURN n limit %s", domain,cqWhere,
                                query.getPageSize());
//                        nodeSql = String.format("MATCH n=(a:'%s')-->() %s RETURN n limit %s", domain,cqWhere,
//                                query.getPageSize());
                    }


                    HashMap<String, Object> graphNode = neo4jUtil.GetGraphNodeAndShip1(nodeSql);
                    ...
                    if (node != null) {
                        ...
                        //MATCH n=(a:法医临床学)-->() where a.name = '%s' RETURN n limit %s
                        //MATCH n=(a:'%s')-->() where a.name = '%s' RETURN n limit %s
                        String nodecql = String.format("MATCH (n:`%s`) %s RETURN n limit %s", domain,
                ...
                } else { // MATCH (n:`%s`) %s RETURN n limit %s   cqWhere, MATCH n=(a:法医临床学)-->() where a.name = %s RETURN n limit %s
                    // 图谱列表导航点击进入


//                    String nodeSql = String.format("MATCH n=(a:法医临床学)-->() where a.name = '%s' RETURN n limit %s", domain,
//                            query.getPageSize());
                    String nodeSql = String.format("MATCH n=(a:法医临床学)-->() where a.name = '%s' RETURN n limit %s", domain,
                            query.getPageSize());
                    List<HashMap<String, Object>> graphNode = neo4jUtil.GetGraphNode(nodeSql);
                    nr.put("node", graphNode);
                    //MATCH (n:`%s`)<-[r]-> (m) %s RETURN r limit %s
                    String domainSql = String.format("MATCH p=(a:法医临床学)<-[r]-> (m) where a.name = '%s' RETURN r limit %s", domain,
                            query.getPageSize());// m是否加领域
            ...
            if (!StringUtil.isBlank(domain)) {
                ...
                if (!StringUtil.isBlank(query.getNodename()) || !StringUtil.isBlank(cqr)) {
                    ...
                    if (node != null) {
                        ...
                        String nodecql = String.format("MATCH (n:`%s`) %s RETURN n limit %s", domain,
                                nodeOnly, query.getPageSize());
                        List<HashMap<String, Object>> nodeItem = neo4jUtil.GetGraphNode1(nodecql);
                        nr.put("node", nodeItem);
                        nr.put("relationship", new ArrayList<HashMap<String, Object>>());
                    }
                } else { // MATCH (n:`%s`) %s RETURN n limit %s   cqWhere,
                    // MATCH n=(a:法医临床学)-->() where a.name = %s RETURN n limit %s
                    // 图谱列表导航点击进入
                    String nodeSql = String.format("MATCH (n:`%s`) %s RETURN n limit %s ", domain, cqWhere,
                            query.getPageSize());
//                    String nodeSql = String.format("MATCH n=(a:法医临床学)-->() where a.name = '%s' RETURN nodes(n) limit %s ", domain, query.getPageSize());
                    List<HashMap<String, Object>> graphNode = neo4jUtil.GetGraphNode1(nodeSql);
                    nr.put("node", graphNode);
                    String domainSql = String.format("MATCH (n:`%s`)<-[r]-> (m) %s RETURN r limit %s", domain,
    ...
    public HashMap<String, Object> getMoreRelationNode(String domain, String nodeid) {
        ...
        try {
            String cypherSql = String.format("MATCH (n:`%s`) -[r]-(m) where id(n)=%s  return * limit 20", domain,
                    nodeid);
            result = neo4jUtil.GetGraphNodeAndShip1(cypherSql);
        } catch (Exception e) {
            e.printStackTrace();
        }
        System.out.println("getMoreRelationNode 346:" + result);
        return result;
    }


    @Override
    public HashMap<String, Object> getmorerelationnodeSec(String sourceName, String TargetName) {
        HashMap<String, Object> result = new HashMap<String, Object>();
        try {
...
//                    "where  p1.name = '%s'  and p2.name = '%s'  return * limit 100", sourceName, TargetName);
            result = neo4jUtil.GetBtnNodeAndShipFuzzy(sourceName, TargetName);
     ...
     */
    @Override
    public HashMap<String, Object> getDetail(String name) {

        HashMap<String, Object> res = new HashMap<>();
        String Cql = String.format("MATCH (n:`%s`)-[r]-(m) return * limit 100", name);

        List<HashMap<String, Object>> hashMaps = neo4jUtil.GetGraphNode(Cql);
        res.put("node", hashMaps);
        return res;
