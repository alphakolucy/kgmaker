var app = new Vue({
    ...
    methods: {
        ...
        //         //         data: data,
        //         //         type: "POST",
        //         //         url: contextRoot + "feNodeDetail",
        //         //         success: function (result) {
        //         //             if (result.code == 200) {
        //         //                 var res = result.data;
        //         //                 _this.detailMsg = res.toString().substring(1, res.length - 2);
        //         //                 alert(_this.detailMsg);
        //         //             }
        //         //         }
        //         //     })
        // },
        ...
        getBtnNodeDetail() {
            var _this = this;
            var data = {inputSource: _this.inputSource, inputTarget: _this.inputTarget};
            $.ajax({
                data: data,
                type: "POST",
                url: contextRoot + "getnodedetail/" + data.inputSource + "/" + data.inputTarget,
                success: function (result) {
                    if (result.code == 200) {
                        _this.editorcontent = result.data.content;
                        _this.showImageList = result.data.imagelist;
        ...
        getdomaingraphSec() {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.code == 200) {
                        var graphModel = result.data;
                        console.log(graphModel.details)
                        if (graphModel != null) {
                            _this.graph.nodes = graphModel.node;
                            _this.graph.links = graphModel.relationship;
                            _this.nodedetail = graphModel.details;
                            // alert(_this.nodedetail)
                            _this.updategraph();
                            // _this.feNodeDetail(_this.ids)
                        }
                    }
                }
            });
        ...
        //     var width = graphcontainer._groups[0][0].offsetWidth;  //.offsetWidth
        ...
        drawlink(link) {
            ...
            linkEnter.on("mouseenter", function (d) {
                d3.select(this).style("stroke-width", "2").attr("stroke", "#ff9e9e").attr("marker-end", "url(#arrow)");
            });
